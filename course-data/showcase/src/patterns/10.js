...
const INTERNAL_REDUCER_TYPES = {
  CLAP: "CLAP",
  RESET: "RESET",
};
const internalReducer = (state, { type, payload }) => {
  switch (type) {
    case INTERNAL_REDUCER_TYPES.CLAP:
      return {
        isClicked: true,
        count: state.count + 1,
    ...
    case INTERNAL_REDUCER_TYPES.RESET:
      return {
        ...payload,
      };
  }
...
const useClapState = (
  initialState = INITIAL_CLAP_STATE,
  reducer = internalReducer
) => {
  const [clapState, dispatch] = useReducer(reducer, initialState);

  const updateClapState = () => dispatch({ type: INTERNAL_REDUCER_TYPES.CLAP });
  ...
  const reset = useCallback(() => {
    if (prevCount !== clapState.count) {
      dispatch({
        type: INTERNAL_REDUCER_TYPES.RESET,
        payload: userInitialState.current,
      });
...
useClapState.types = INTERNAL_REDUCER_TYPES;
...
const MediumClap = () => {
  ...
  const reducer = (state, action) => {
    if (
      action.type === useClapState.types.CLAP &&
      state.count == MAXIMUM_CLAP
    ) {
      return state;
    }
